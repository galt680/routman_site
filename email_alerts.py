...
def new_watchlist_2017(test = False):
    today = datetime.date.today()
    alert_overbought = []
    alert_oversold = []
    rsi_overbought = []
    ...
    with con:
        cur = con.cursor()
        cur.execute("SELECT * FROM watchlist_hurry WHERE SIGNAL != 0 ORDER BY Name Asc")
        rows = cur.fetchall()
        for name,signal,rsi,slowk,slowd in rows:
            if signal == -1:
                alert_overbought.append("%s is overbought with RSI at %s and the stochastics at %s and at %s"%(name,rsi,slowk,slowd))
        ...
        cur.execute("SELECT * FROM watchlist_hurry WHERE RSI NOT BETWEEN 35 AND 65 ORDER BY RSI DESC")
        rows = cur.fetchall()
        for name,signal,rsi,slowk,slowd in rows:
            if rsi > 65:
                rsi_overbought.append("%s is overbought with RSI at %s"%(name,rsi))
    ...
    message = """Good evening,\n(This is an automated message)\n\nThese are the stocks that are currently overbought and oversold from your new positions watchlist:\n"""
    for i in alert_overbought:
        message +='\n %s'%i
    message +="""\n\nThese are the stocks that are currently oversold from your new positions watchlist:\n"""
    for i in alert_oversold:
        message += '\n %s'%i
    message +="""\n\nThese are the stocks that are currently overbought based only on RSI from your new positions watchlist:\n"""
    for i in rsi_overbought:
        message += '\n %s'%i
    message +="""\n\nThese are the stocks that are currently oversold based only on RSI from your new positions watchlist:\n"""
    for i in rsi_oversold:
        message += '\n %s'%i
    message += "\n\n If you have any adjustments you'd like to make to the alert settings or if you'd like to add symbols to the new positions watchlist please let me know.\nYisroel Schaffel"
    ...
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "new Positions Watchlist Alerts for %s"%today.strftime("%m-%d-%Y")
